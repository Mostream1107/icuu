...
export function convertToYoloV8Config(data, hyper_parameter, filePath) {
    const ncValue = hyper_parameter.find(param => param.label === 'nc').value;
...
scales:  # 模型复合缩放常数
  ...
  n: [0.33, 0.25, 1024]  # YOLOv8n概览
  s: [0.33, 0.50, 1024]  # YOLOv8s概览
  m: [0.67, 0.75, 768]   # YOLOv8m概览
  l: [1.00, 1.00, 512]   # YOLOv8l概览
  x: [1.00, 1.25, 512]   # YOLOv8x概览`;
    const backbone = data.backbone.map(item => {
        const line = item.map(innerItem => {
            if (Array.isArray(innerItem)) {
                return `[${innerItem.join(', ')}]`;
            }
            return innerItem;
        }).join(', ');
        return `  - [${line}]`;
    }).join('\n');
    const head = data.head.map(item => {
        const line = item.map(innerItem => {
            if (Array.isArray(innerItem)) {
                return `[${innerItem.join(', ')}]`;
            }
